...
const Dashboard = ({ isOnline, setIsOnline }) => {
  ...
  // 💡 Use a ref to always have the absolute latest state inside socket listeners
  const incomingRideRef = useRef(null);
  useEffect(() => {
    incomingRideRef.current = incomingRide;
  }, [incomingRide]);

  // ⚡ ACTIVE TRIP LAYER STATE
  ...
  /* 🔁 2. CHANNELS SYNC ENGINE WITH RECONNECT FALLBACKS */
  useEffect(() => {
    const handleConnect = () => {
  ...
  /* 📍 3. GEOLOCATION TELEMETRY HEARTBEAT ENGINE */
  useEffect(() => {
  ...
  /* 🚕 4. DYNAMIC GEOLOCATION TRANSLATION & EVENT CAPTURE ENGINE */
  useEffect(() => {
    // 💡 Kept clean of dynamic state arrays so listeners are set up ONLY ONCE
    const onRideRequest = async (payload) => {
      ...
      try {
        ...
        console.error("⚠️ Address translation fallback executed:", err.message);
    ...
    const onRideConfirmed = (data) => {
      // 💡 Use the ref here to prevent reading stale/null state definitions!
  ...
  }, []); // 🟢 Notice dependency array is empty now. Pure connection stability.

  /* ✅ 5. DISPATCH ACTION CONTROL FLOW */
  ...
  return (
    <>
      ...
      {/* 🚨 DYNAMIC INCOMING RIDE NOTIFICATION MODAL OVERLAY */}
      ...
      {/* 🗺️ ACTIVE NAVIGATION ROUTER CONTROL CENTER */}
      {activeTrip && (
